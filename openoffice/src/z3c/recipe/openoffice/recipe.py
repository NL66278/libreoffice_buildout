...
class Recipe(object):
    def __init__(self, buildout, name, options):
        ...
        options.setdefault(
            'download-url',
            'http://downloads.sourceforge.net/project/openofficeorg.mirror/'
            '4.1.2/binaries/en-US/'
            'Apache_OpenOffice_4.1.2_Linux_x86-64_install-rpm_en-US.tar.gz'
        )
        options.setdefault(
            'unpack-name',
            'en-US')
    ...
    def copy(self, storage):
        ...
        shutil.copytree(
            os.path.join(storage, 'opt', 'openoffice4'),
            location
        )
        return True
    ...
    def hack_python(self):
        ...
        location = self.options['location']
        program_dir = os.path.join(location, 'program')
        os.remove(os.path.join(program_dir, 'libpython2.7.so.1.0'))
        shutil.rmtree(os.path.join(program_dir, 'python-core-2.7.6'))
        ...
        os.symlink(so, os.path.join(program_dir, 'libpython2.7.so.1.0'))
